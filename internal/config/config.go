...
type Config struct {
	...
	ProdBackendURL     string `yaml:"prod_backend_url"`
...
// SaveConfig marshals cfg and writes it to config.yaml in configDir.
func SaveConfig(configDir string, cfg *Config) error {
	...
	return os.WriteFile(
		filepath.Join(configDir, "config.yaml"), data, 0600)
}
